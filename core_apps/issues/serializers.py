...
class IssueSerializer(serializers.ModelSerializer):
  ...
  def get_view_count(self, obj):
    ...
    return ContentView.objects.filter(object_id=obj.pkid, content_type=content_type).count()
...
class IssueStatusUpdateSerializer(serializers.ModelSerializer):
  ...
  def update(self, instance: Issue, validated_data: dict) -> Issue:
    if (validated_data.get("status")==Issue.Status.RESOLVED and instance.status!=Issue.Status.RESOLVED):
